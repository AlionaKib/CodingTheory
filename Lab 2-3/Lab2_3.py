...
#Подключение библиотек для работы с массивами и битовыми массивами

class CyclicCodes:

#Инициализация переменных k - число информационных разрядов
# n - длина закодированного сообщения
# g - образующий многочлен
# t - кратность ошибки
...
#Умножение входного двоичного массива input на образующий многочлен gen
...
#Деление с остатком входного двоичного массива input на образующий многочлен gen
...
#Систематическое кодирование входного двоичного массива input
...
#Формирование таблицы синдромов для всех ошибок кратности не выше t
    def make_table(self):
        syndromes = {}
        codes = []
...
#Вычисление размера битового массива
size_bitarray = 8 * 8 * cc.k // cc.n

with open(path_to_input_file, 'wb') as file:
    # создание bitarray с определённым количеством нулей
    file_bitarray = ba.bitarray('0' * size_bitarray)
    for i in range(0, len(file_bitarray)):
        # Заполнение bitarray рандомными значениями
        file_bitarray[i] = np.random.randint(0, 2) == 1
    # Запись псевдослучайный bitarray в файл
    file_bitarray.tofile(file)
#Проверка работоспособности методов, созданных ранее
