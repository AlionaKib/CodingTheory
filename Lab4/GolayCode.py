...
#Умножение входного двоичного массива input на порождающую матрицу
    def encode(self, input):
        # Кодировка сигнала
        if len(input) != self.k:
            print("Error!")
            return None
        # Единичная матрица из нулей и единиц
        I = np.eye(self.k)
        # Матрица G, состоящая из нулей и единиц (G = [I B])
        G = np.concatenate([I, self.B], axis=1)
        # Если передаваемый параметр - bitarray, то вызываем tolist()
        if input.__class__ == ba.bitarray().__class__:
            np_input = np.array(input.tolist())
        else:
            np_input = np.array(input)
        encode = np.dot(np_input, G) % 2
        # Возвращение булевого массива, содержащего закодированный сигнал
        return np.array(encode, dtype=bool)

    def decode(self, input_n):
        # Декодирование сигнала
        # Единичная матрица из нулей и единиц
        I = np.eye(self.k)
        # Матрица H, состоящая из нулей и единиц (H = [ I ])
        #                                             [ B ]
